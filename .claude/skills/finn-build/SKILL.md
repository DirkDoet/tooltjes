...
Using the Linear connector, list issues on team `DIR` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `DIR-NNN-short-slug`, using the issue's real identifier.
...
- `Closes DIR-NNN`, using the real Linear issue identifier
